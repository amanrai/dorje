...
@tool(description="Placeholder PDF-to-Markdown extractor. Requires a future PDF backend before it can emit derivatives.", requires="file_ref:application/pdf", produces="extracted_markdown")
...
@tool(description="Placeholder PDF image extractor for actual embedded/raster image payloads. Requires a future PDF image backend before it can emit derivatives.", requires="file_ref:application/pdf", produces="collection/image")
...
@tool(description="Placeholder PDF figure extractor for document-level figure records with captions/labels/references, not raw image bytes. Requires a future PDF figure backend before it can emit derivatives.", requires="file_ref:application/pdf", produces="collection/figure")
...
@tool(description="Placeholder PDF table extractor. Requires a future PDF table backend before it can emit derivatives.", requires="file_ref:application/pdf", produces="collection/table")
